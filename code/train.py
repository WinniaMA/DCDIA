...
from src.get_node_features import graph_batch_from_smile_molerec
from src.models import DCDIA
...
def eval(model, data_eval, voc_size, epoch,drug_data):
    ...
    for step, input in enumerate(data_eval):
        ...
        for adm_idx, adm in enumerate(input):
            ...
            target_output1 = model(input[:adm_idx+1],**drug_data)
...
def main():
    ...
    node_features_path = '../data/node_features.pkl'
    substruct_smile_path = '../data/substructure_smiles.pkl'
    device = torch.device('cuda:0')

    ddi_mask_path = '../data/ddi_mask_H.pkl'  # sub_structure_module
    ...
    ddi_mask_H = dill.load(open(ddi_mask_path, 'rb'))    # sub_structure_module
    substruct_smiles_list = dill.load(open(substruct_smile_path, 'rb'))  # sub_structure_module
    substruct_graphs = graph_batch_from_smile_molerec(substruct_smiles_list) # sub_structure_module


    node_features = dill.load(open(node_features_path, 'rb'))
    ...
    substruct_para = {
        'num_layer': 3, 'emb_dim': 64, 'graph_pooling': 'mean',
        'drop_ratio': 0.85, 'gnn_type': 'gin', 'virtual_node': False
    }
    ...
    model = DCDIA(voc_size,
                    ...
                    ddi_in_memory=DDI_IN_MEM)  # sub_structure_module
    ...
    if TEST:
        eval(model, data_test, voc_size, 0, drug_data)
    else:
        ...
        for epoch in range(EPOCH):
            ...
            for step, input in enumerate(data_train):
                for idx, adm in enumerate(input):
                    ...
                    target_output1, batch_neg_loss = model(seq_input,**drug_data) # sub_structure_module
                    ...
                    if Neg_Loss:
                        ...
                        # print("current_ddi_rate", current_ddi_rate)
                        if current_ddi_rate <= TARGET_DDI:
                            loss = 0.9 * loss1 + 0.01 * loss3
                            # loss = 0.95 * loss1 + 0.05 * loss3
                            prediction_loss_cnt += 1
                           # print(" prediction_loss_cnt",  prediction_loss_cnt)
                        else:
            ...
            ddi_rate, ja, prauc, avg_p, avg_r, avg_f1 = eval(model, data_eval, voc_size, epoch, drug_data)
